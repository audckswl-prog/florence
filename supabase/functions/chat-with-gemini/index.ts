...
serve(async (req) => {
  ...
  try {
    ...
    if (!title || !author || !description) {
      throw new Error("Missing required parameters: title, author, description");
    }
...
4. 분량은 전체 글자 수 기준 600~700자 내외의 충분히 깊이있는 2~3문단으로 써. 분량이 너무 짧으면 안 돼.
...

{
  ...
  "historical_background": "맞춤형 분석 내용 (고전은 시대배경과 현대의 연결 / 신작실용서는 최신 트렌드와 현실적 통찰) (흡입력 있는 2~3문단, 600~700자 내외)",
...
`;
    ...
    // Parse out potential markdown formatting
    const jsonBlockMatch = textResult.match(/\{[\s\S]*\}/);
    if (jsonBlockMatch) {
      textResult = jsonBlockMatch[0];
    }
