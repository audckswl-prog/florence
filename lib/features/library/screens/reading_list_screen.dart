    ...
    return DefaultTabController(
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          backgroundColor: AppColors.ivory,
          elevation: 0,
          toolbarHeight: 0, // Hide default title area to only show custom bottom
        ...
        body: booksAsync.when(
          data: (books) {
            ...
            return TabBarView(
              children: [
                _BookList(books: readingBooks, emptyMessage: '읽고 있는 책이 없습니다.', type: 'reading'),
                _BookList(books: wishBooks, emptyMessage: '찜한 책이 없습니다.', type: 'wish'),
              ],
            );
          },
            ...
            child: NeumorphicContainer(
              ...
              child: Row(
                children: [
                       ...
                       children: [
                         ...
                         if (type == 'reading' && book.startedAt != null) ...[
                            ...
                            Text(
                              ...
                              style: const TextStyle(fontSize: 12, color: AppColors.burgundy),
                            ),
                         ],
                       ],
                     ),
                   ...
                   if (type == 'wish')
                     IconButton(
                       icon: const Icon(Icons.import_contacts_rounded, color: AppColors.burgundy, size: 28),
                       tooltip: '읽기 시작',
                       onPressed: () async {
                         ...
                         if (confirmed == true) {
                           ...
                           if (userId != null) {
                             ...
                             ref.invalidate(userBooksProvider); // Fix: refresh root provider 
                             if (context.mounted) {
                               FlorenceToast.show(context, '읽는 중인 책 목록으로 이동되었습니다.');
                             }
                           }
                         }
                       },
                   ...
                   if (type == 'reading')
                     IconButton(
                       icon: const Icon(Icons.library_add_check_rounded, color: AppColors.burgundy, size: 28),
                       ...
                       onPressed: () {
                         showDialog(
                           ...
                           builder: (context) => ReadingRecordDialog(
                             userBook: book,
                             onConfirm: (readPages, totalPages, readCount) async {
                               final userId = Supabase.instance.client.auth.currentUser?.id;
                               if (userId != null) {
                                 ...
                                 ref.invalidate(userBooksProvider); // Update library and all dependent lists
                                 ...
                                 final quote = await showDialog<String>(
                                   context: context,
                                   builder: (context) => ReadingCompletionDialog(userBook: updatedBook),
                                 );
